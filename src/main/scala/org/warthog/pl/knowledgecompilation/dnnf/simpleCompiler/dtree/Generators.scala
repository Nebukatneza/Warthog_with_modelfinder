...
object Generators {
   ...
   */
  def generateDTree(clauses: List[Set[Lit]]): DTree = {
    ...
    for (i <- 0 until dTreeLines.size)
      dTreeTmp(i) = dTreeLines(i)(0) match {
        case 'L' =>
          ...
          DTreeLeaf(n, dimacsLines(n).split(" ").map(_.toInt).filterNot(_ == 0).map(v => Lit(math.abs(v), v > 0)).toSet)
        case 'I' =>
